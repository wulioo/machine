...
def cal_td_ndcg(n_group: int, td_df: pd.DataFrame, factor_col: str, label_col: str) -> (float, float):
    ...
    # 计算当天每层 instrument 个数和平均收益率
    instr_per_group = len(td_df) / n_group
    avg_ret_long = td_df[label_col].mean()
    avg_ret_short = - avg_ret_long

    # 多头、空头分别对收益率按因子值降序、自降序排列
    label_long = td_df.sort_values(by=factor_col, ascending=False, ignore_index=True)[label_col]
    label_long_ideal = td_df[label_col].sort_values(ascending=False, ignore_index=True)
    label_short = - td_df.sort_values(by=factor_col, ascending=True, ignore_index=True)[label_col]
    label_short_ideal = - td_df[label_col].sort_values(ascending=True, ignore_index=True)
    ...
    for i in range(n_group):
        ...
        group_df.loc[i, 'long'] = label_long.iloc[index_start:index_end].mean() - avg_ret_long
        group_df.loc[i, 'long_ideal'] = label_long_ideal.iloc[index_start:index_end].mean() - avg_ret_long
        group_df.loc[i, 'short'] = label_short.iloc[index_start:index_end].mean() - avg_ret_short
        group_df.loc[i, 'short_ideal'] = label_short_ideal.iloc[index_start:index_end].mean() - avg_ret_short
...
def cal_avg_ndcg(n_group: int, df: pd.DataFrame, factor_col: str, label_col: str, start=None, end=None):
    ...
    ndcg_df.index.rename('trading_date', inplace=True)
